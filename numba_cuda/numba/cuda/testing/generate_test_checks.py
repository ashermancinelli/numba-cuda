#!/usr/bin/env python3
"""Adapted from generate-test-checks.py in the MLIR LLVM subproject
for use in Numba CUDA.

This script is a utility to add FileCheck patterns to an LLVM IR file.
...
# Modified to accept quote characters in the SSA name.
SSA_RE_STR = r'[0-9]+|["a-zA-Z$._-]["a-zA-Z0-9$._-]*'
...
class VariableNamer:
    def __init__(self, variable_names: str):
        ...
        self.generate_in_parent_scope_left: int = 0

        # Parse variable names
        self.variable_names: list[str] = [
            name.upper() for name in variable_names.split(",")
        ]
        self.used_variable_names: set[str] = set()

    # Generate the following 'n' variable names in the parent scope.
    def generate_in_parent_scope(self, n: int):
        self.generate_in_parent_scope_left = n

    # Generate a substitution name for the given ssa value name.
    def generate_name(
        self, source_variable_name: str, use_ssa_name: bool
    ) -> str:
...
class AttributeNamer:
    def __init__(self, attribute_names: str):
        self.name_counter: int = 0
        self.attribute_names: list[str] = [
            name.upper() for name in attribute_names.split(",")
        ]
        self.map: dict[str, str] = {}
        self.used_attribute_names: set[str] = set()

    # Generate a substitution name for the given attribute name.
    def generate_name(self, source_attribute_name: str) -> str:
    ...
    def get_name(self, source_attribute_name: str) -> str | None:
...
def get_num_ssa_results(input_line: str) -> int:
...
def process_line(
    ...
    use_ssa_name: bool = False,
    strict_name_re: bool = False,
    wildcard_ssa_names: bool = True,
):
    output_line = ""
...
def process_source_lines(
    source_lines: list[str], note: str, args: argparse.Namespace
) -> list[list[str]]:
    source_split_re = re.compile(args.source_delim_regex)

    source_segments: list[list[str]] = [[]]
...
def process_attribute_definition(
    line: str, attribute_namer: AttributeNamer
) -> str | None:
...
def process_attribute_references(
    line: str, attribute_namer: AttributeNamer
) -> str:
    output_line = ""
...
def main():
    ...
    _ = parser.add_argument("input", nargs="?", type=Path, default=sys.stdin)
    _ = parser.add_argument(
        "--source",
        type=str,
    ...
    _ = parser.add_argument("--source_delim_regex", type=str, default="func @")
    _ = parser.add_argument(
        "--starts_from_scope",
        type=int,
        default=1,
    ...
    _ = parser.add_argument(
        "-i", "--inplace", action="store_true", default=False
    )
    _ = parser.add_argument(
        "--variable_names",
        type=str,
    ...
    _ = parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="If true, matches will save the SSA register name and use it for further matches. "
        + "If false, matches will use wildcards for every SSA register check.",
    )
    ...
    input_lines: list[str] = [
        l.rstrip() for l in args.input.read_text().splitlines()
    ]
    ...
    if args.source:
        source_segments = process_source_lines(
            [l.rstrip() for l in Path(args.source).read_text().splitlines()],
            autogenerated_note,
            args,
        )
    ...
    if args.inplace:
        ...
        assert source and source is not sys.stdin, (
            "To write the checks in-place, either specify the source "
            + "file or pass input as a positional argument"
        )
    ...
    for input_line in input_lines:
        ...
        if len(output_segments[-1]) != 0 or not ssa_split[0]:
            ...
            output_line += process_line(
                ssa_split[1:],
                variable_namer,
                wildcard_ssa_names=args.wildcard_ssa_names,
            )
    ...
    if source_segments:
        ...
        for check_segment, source_segment in zip(
